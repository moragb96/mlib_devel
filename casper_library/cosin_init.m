...
function cosin_init(blk,varargin)
  ...
  invert_gen([blk,'/invert0'],floating_point);
  ...
  invert_gen([blk,'/invert1'],floating_point);
...
function add_convert_init(blk, full_cycle_bits, address_bits, lookup_bits, lookup_function, output)
  ...
  reuse_block(blk, 'Delay14', 'xbsIndex_r4/Delay', ...
    'reg_retiming', 'off', 'latency', 'add_latency', 'Position', [540 201 570 219]);    
...
function invert_gen(blk,floating_point)

	invert_mask(blk);
	invert_init(blk,floating_point);
end % invert_gen
...
function invert_init(blk,floating_point)
