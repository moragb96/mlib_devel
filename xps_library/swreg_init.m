...
function draw_to()
    ...
    reuse_block(blk, gwout_name, 'xbsIndex_r4/Gateway Out', ...
            ...
            'hdl_port', 'on');
    if simport
        % For each input, add a simulation-only gateway out and feed it to a port
        for pindex = 1 : numios
            out_name = sprintf('sim_out_%s', current_names{pindex});
            % Compensate for the delays specified by the user (+1 hardcoded for the cast)
            % Let the delays be behavioural. Assume the compiler will optimize them away. (I hate this)
            reuse_block(blk, [out_name '_sim_delay'], 'xbsIndex_r4/Delay', ...
                       'latency', num2str(1 + str2double(get_param(blk, 'io_delay'))), ...
                       'reg_retiming', 'on', ...
                        'Position', [x_start + (x_size), ...
                        y_pos_row + 4*pindex*y_size, ...
                        x_start + (x_size) + (x_size/2), ...
                        y_pos_row + (4*pindex+1)*y_size]);
            reuse_block(blk, [out_name '_gw'], 'xbsIndex_r4/Gateway Out', ...
                'Position', [x_start + (x_size * 4 * 2), ...
                             y_pos_row + 4*pindex*y_size, ...
                             x_start + (x_size * 4 * 2) + (x_size/2), ...
                             y_pos_row + (4*pindex+1)*y_size], ...
                'hdl_port', 'off');
            reuse_block(blk, out_name, 'built-in/outport', 'Port', '1', ...
                'Position', [x_start + (x_size * 6 * 2), y_pos_row + 4*pindex*y_size, ...
                             x_start + (x_size * 6 * 2) + (x_size/2), y_pos_row + (4*pindex+1)*y_size]);
            add_line(blk, [out_name, '_sim_delay/1'], [out_name, '_gw/1'], 'autorouting', 'on');
            add_line(blk, [out_name, '_gw/1'], [out_name, '/1'], 'autorouting', 'on');
        end
    end
    % Terminate the real output gateway
    ...
    for pindex = 1 : numios
        ...
        if simport
            % connect the inputs directly to the gateway out.
            % That way we don't have to mess around with data types
            add_line(blk, [in_name, '/1'], [out_name, '_sim_delay/1'], 'autorouting', 'on');
        end
